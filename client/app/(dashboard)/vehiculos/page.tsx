...
export default function VehiculosPage() {
  ...
  const openMenuFor = (vehicleId: number, event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handleToggleActivo = async (vehicle: VehicleListItem) => {
    closeMenu();
    const activating = !vehicle.activo;

    if (!activating) {
      const confirmed = await showConfirm({
        title: 'Desactivar vehículo',
        text: `¿Seguro que querés desactivar ${vehicle.marca.marca} ${vehicle.marca.modelo}?`,
    ...
    try {
      ...
      showSuccess(
        activating ? 'Vehículo activado' : 'Vehículo desactivado',
        `El vehículo fue ${activating ? 'activado' : 'desactivado'} correctamente.`,
      );
      await loadVehicles();
    } catch (err) {
      showError(
        activating ? 'No se pudo activar el vehículo' : 'No se pudo desactivar el vehículo',
        err instanceof Error ? err.message : 'No se pudo conectar con el servidor.',
      );
    } finally {
      setTogglingId(null);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">
            Gestioná los vehículos del sistema.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          <span className="rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700">
            {activeCount} vehículo{activeCount === 1 ? '' : 's'} activo
          ...
          <table className="min-w-full divide-y divide-stone-200">
            ...
            <tbody className="divide-y divide-stone-100">
              {vehicles.map((vehicle, index) => (
                <tr key={vehicle.id} className="hover:bg-stone-50/60">
                  ...
                  <td className="px-4 py-3 text-center text-sm text-stone-600">
                    {vehicle.kilometraje.toLocaleString('es-AR')}
                  </td>
                  <td className="px-4 py-3 text-center text-sm text-stone-600">{vehicle.cliente.razonSocial}</td>
                  <td className="px-4 py-3 text-center text-sm text-stone-600">
                    <span
                  ...
                  <td className="px-4 py-3 text-center text-sm">
                    <div
                      ...
                      ref={openMenuId === vehicle.id ? triggerRef : null}
                    >
                      <button
                        type="button"
                        onClick={(event) => openMenuFor(vehicle.id, event)}
                      ...
                      {openMenuId === vehicle.id &&
                        ...
                        createPortal(
                          ...
                          >
                            <Link
                              href={`/vehiculos/editar/${vehicle.id}`}
                              onClick={closeMenu}
                            ...
                            <button
                              type="button"
                              onClick={() => handleToggleActivo(vehicle)}
        ...
        {!loading && !listError && vehicles.length > 0 && (
          <div className="flex flex-col items-center justify-between gap-3 border-t border-stone-200 px-4 py-3 text-sm text-stone-500 sm:flex-row">
            <span>
              ...
              {Math.min(page * pageSize, total)} de {total} vehículos
            </span>
