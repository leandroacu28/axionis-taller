...
const ESTADO_BADGE_CLASSES: Record<Estado, string> = {
  pendiente: 'bg-blue-600 text-white dark:bg-blue-600 dark:text-white',
  en_proceso: 'bg-yellow-600 text-white dark:bg-yellow-600 dark:text-white',
  terminado: 'bg-green-600 text-white dark:bg-green-600 dark:text-white',
  cancelado: 'bg-red-600 text-white dark:bg-red-600 dark:text-white',
};

function SquaresIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="h-7 w-7 shrink-0" aria-hidden="true">
...
export default function PanelStats({ stats, total }: { stats: PanelStatsData; total: number }) {
  const figures: Figure[] = [
    ...
    // que siempre mide el día calendario actual.
    {
      ...
      value: total,
      badgeClass: 'bg-stone-600 text-white dark:bg-stone-600 dark:text-white',
      cardBgClass: 'bg-stone-100 dark:bg-stone-800',
      unit: ordenesUnit,
      icon: <SquaresIcon />,
    ...
    },
    {
      ...
      cardBgClass: 'bg-blue-100 dark:bg-blue-500/15',
      unit: ordenesUnit,
      icon: <ClockIcon />,
    ...
    },
    {
      ...
      cardBgClass: 'bg-yellow-100 dark:bg-yellow-500/15',
      unit: ordenesUnit,
      icon: <WrenchIcon />,
    ...
    },
    {
      ...
      cardBgClass: 'bg-green-100 dark:bg-green-500/15',
      unit: ordenesUnit,
    ...
    },
    {
      ...
      badgeClass: 'bg-purple-600 text-white dark:bg-purple-600 dark:text-white',
      cardBgClass: 'bg-purple-100 dark:bg-purple-500/15',
      unit: mecanicosUnit,
      icon: <UsersIcon />,
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:grid-cols-5">
      {figures.map((figure) => (
        <div
          key={figure.label}
          className={`flex flex-col gap-2 rounded-xl border border-stone-200 p-4 shadow-sm transition-colors dark:border-stone-700 ${figure.cardBgClass}`}
        >
          <div className="flex items-center justify-between gap-2">
            <span className={`w-fit rounded-full px-2.5 py-1 text-sm font-semibold ${figure.badgeClass}`}>
              {figure.label}
            </span>
            <span className={figure.iconClass}>{figure.icon}</span>
          </div>
          <span className="flex items-baseline gap-1.5">
            <span className="text-3xl font-bold text-stone-900 dark:text-stone-50">{figure.value}</span>
            <span className="text-base font-bold text-stone-500 dark:text-stone-400">{figure.unit(figure.value)}</span>
          </span>
        </div>
      ))}
    </div>
