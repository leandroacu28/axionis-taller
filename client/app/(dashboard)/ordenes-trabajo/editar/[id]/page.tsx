...
import {
  getOrdenTrabajo,
  updateOrdenTrabajo,
  type UpdateOrdenTrabajoPayload,
...
export default function EditarOrdenTrabajoPage({ params }: { params: { id: string } }) {
  ...
  const [initialValues, setInitialValues] = useState<OrdenTrabajoFormValues | null>(null);
  ...
  useEffect(() => {
    ...
    const loadOrden = async () => {
      ...
      setLoadError('');
      try {
        const orden = await getOrdenTrabajo(ordenId);
        if (cancelled) return;
  ...
  const handleSubmit = async (payload: UpdateOrdenTrabajoPayload) => {
    if (submitting) return;
    setSubmitting(true);
    try {
      await updateOrdenTrabajo(ordenId, payload);
