...
export default function UnidadesMedidaPage() {
  const [unidadesMedida, setUnidadesMedida] = useState<UnidadMedidaListItem[]>([]);
  ...
  const [selectedUnidadMedida, setSelectedUnidadMedida] = useState<UnidadMedidaListItem | null>(null);
  ...
  const openMenuFor = (unidadMedidaId: number, event: React.MouseEvent<HTMLButtonElement>) => {
    if (openMenuId === unidadMedidaId) {
      closeMenu();
      return;
    }
  ...
  const handleToggleActivo = async (unidadMedida: UnidadMedidaListItem) => {
    closeMenu();
    const activating = !unidadMedida.activo;

    if (!activating) {
      const confirmed = await showConfirm({
        title: 'Desactivar unidad de medida',
        text: `¿Seguro que querés desactivar ${unidadMedida.descripcion}?`,
    ...
    setTogglingId(unidadMedida.id);
    try {
      await updateUnidadMedida(unidadMedida.id, {
        descripcion: unidadMedida.descripcion,
        activo: activating,
      });
      showSuccess(
        activating ? 'Unidad de medida activada' : 'Unidad de medida desactivada',
        `${unidadMedida.descripcion} fue ${activating ? 'activada' : 'desactivada'} correctamente.`,
      );
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">
            Gestioná las unidades de medida del sistema.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          <span className="rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700">
            {activeCount} unidad{activeCount === 1 ? '' : 'es'} de medida activa
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
        {loading ? (
          ...
          <table className="min-w-full divide-y divide-stone-200">
            ...
            <tbody className="divide-y divide-stone-100">
              {unidadesMedida.map((unidadMedida, index) => (
                <tr key={unidadMedida.id} className="hover:bg-stone-50/60">
                  ...
                  <td className="px-4 py-3 text-center text-sm text-stone-600">
                    {unidadMedida.creadoPor
                      ? [unidadMedida.creadoPor.nombre, unidadMedida.creadoPor.apellido].filter(Boolean).join(' ') ||
                        unidadMedida.creadoPor.username
                      : '—'}
                    <span className="block text-xs text-stone-400">
                      {new Date(unidadMedida.createdAt).toLocaleString('es-AR', {
                        dateStyle: 'short',
                        timeStyle: 'short',
                      })}
                    </span>
                  ...
                  <td className="px-4 py-3 text-center text-sm">
                    <div
                      ...
                      ref={openMenuId === unidadMedida.id ? triggerRef : null}
                    >
                      <button
                        type="button"
                        onClick={(event) => openMenuFor(unidadMedida.id, event)}
                        aria-label="Abrir acciones"
                        className="rounded-lg px-2 py-1 text-lg leading-none text-stone-500 hover:bg-stone-100 hover:text-stone-700"
                      >
                        ⋯
                      </button>
                      {openMenuId === unidadMedida.id &&
                        menuPos &&
                        ...
                        createPortal(
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleToggleActivo(unidadMedida)}
        ...
        {!loading && !listError && unidadesMedida.length > 0 && (
          <div className="flex flex-col items-center justify-between gap-3 border-t border-stone-200 px-4 py-3 text-sm text-stone-500 sm:flex-row">
            <span>
              ...
              {Math.min(page * pageSize, total)} de {total} unidades de medida
            </span>
