...
import {
  ...
  type CreateUnidadMedidaPayload,
  type UpdateUnidadMedidaPayload,
...
interface UnidadMedidaFormModalProps {
  open: boolean;
  onClose: () => void;
  unidadMedida: UnidadMedidaListItem | null;
  onSaved: () => void;
}

export default function UnidadMedidaFormModal({ open, onClose, unidadMedida, onSaved }: UnidadMedidaFormModalProps) {
  const isEdit = unidadMedida !== null;
  ...
  useEffect(() => {
    if (!open) return;
    const loaded: FormState = unidadMedida
      ? { descripcion: unidadMedida.descripcion, activo: unidadMedida.activo }
      : EMPTY_FORM;
    setForm(loaded);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit && unidadMedida) {
        const payload: UpdateUnidadMedidaPayload = {
          descripcion: form.descripcion,
          activo: form.activo,
        };
        await updateUnidadMedida(unidadMedida.id, payload);
        showSuccess('Unidad de medida actualizada', 'Los cambios se guardaron correctamente.');
      } else {
        const payload: CreateUnidadMedidaPayload = {
          descripcion: form.descripcion,
        };
        await createUnidadMedida(payload);
        showSuccess('Unidad de medida creada', 'La unidad de medida ha sido creada correctamente.');
      }
      onSaved();
    } catch (err) {
      showError(
        isEdit ? 'Error al actualizar unidad de medida' : 'Error al crear unidad de medida',
  ...
  return (
    <Modal
      ...
      description={
        isEdit
          ? 'Modificá los datos de la unidad de medida.'
          : 'Completá los datos para registrar una unidad de medida.'
      }
    >
