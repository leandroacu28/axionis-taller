...
import { listBrands, updateBrand, exportBrands, type BrandListItem } from '../../lib/brands';
...
export default function MarcasPage() {
  ...
  const openMenuFor = (brandId: number, event: React.MouseEvent<HTMLButtonElement>) => {
    if (openMenuId === brandId) {
      closeMenu();
      return;
    }
  ...
  const handleToggleActivo = async (brand: BrandListItem) => {
    closeMenu();
    const activating = !brand.activo;

    if (!activating) {
      const confirmed = await showConfirm({
        title: 'Desactivar marca',
        text: `¿Seguro que querés desactivar ${brand.marca} ${brand.modelo}?`,
    ...
    try {
      ...
      showSuccess(
        activating ? 'Marca activada' : 'Marca desactivada',
        `${brand.marca} ${brand.modelo} fue ${activating ? 'activada' : 'desactivada'} correctamente.`,
      );
      await loadBrands();
    } catch (err) {
      showError(
        activating ? 'No se pudo activar la marca' : 'No se pudo desactivar la marca',
        err instanceof Error ? err.message : 'No se pudo conectar con el servidor.',
      );
    } finally {
      setTogglingId(null);
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">
            Gestioná las marcas del sistema.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          <span className="rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700">
            {activeCount} marca{activeCount === 1 ? '' : 's'} activa
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
          ...
          <table className="min-w-full divide-y divide-stone-200">
            ...
            <tbody className="divide-y divide-stone-100">
              {brands.map((brand, index) => (
                <tr key={brand.id} className="hover:bg-stone-50/60">
                  ...
                  <td className="px-4 py-3 text-center text-sm">
                    <div
                      ...
                      ref={openMenuId === brand.id ? triggerRef : null}
                    >
                      <button
                        type="button"
                        onClick={(event) => openMenuFor(brand.id, event)}
                      ...
                      {openMenuId === brand.id &&
                        ...
                        createPortal(
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleToggleActivo(brand)}
        ...
        {!loading && !listError && brands.length > 0 && (
          <div className="flex flex-col items-center justify-between gap-3 border-t border-stone-200 px-4 py-3 text-sm text-stone-500 sm:flex-row">
            <span>
              ...
              {Math.min(page * pageSize, total)} de {total} marcas
            </span>
