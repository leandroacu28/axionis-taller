...
import { listColors, updateColor, exportColors, type ColorListItem } from '../../lib/colors';
...
export default function ColoresPage() {
  ...
  const openMenuFor = (colorId: number, event: React.MouseEvent<HTMLButtonElement>) => {
    if (openMenuId === colorId) {
      closeMenu();
      return;
    }
  ...
  const handleToggleActivo = async (color: ColorListItem) => {
    closeMenu();
    const activating = !color.activo;

    if (!activating) {
      const confirmed = await showConfirm({
        title: 'Desactivar color',
        text: `¿Seguro que querés desactivar ${color.descripcion}?`,
    ...
    try {
      ...
      showSuccess(
        activating ? 'Color activado' : 'Color desactivado',
        `${color.descripcion} fue ${activating ? 'activado' : 'desactivado'} correctamente.`,
      );
      await loadColors();
    } catch (err) {
      showError(
        activating ? 'No se pudo activar el color' : 'No se pudo desactivar el color',
        err instanceof Error ? err.message : 'No se pudo conectar con el servidor.',
      );
    } finally {
      setTogglingId(null);
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-stone-500 dark:text-stone-400">
            Gestioná los colores del sistema.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          <span className="rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700">
            {activeCount} color{activeCount === 1 ? '' : 'es'} activo
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
        ...
        ) : colors.length === 0 && total === 0 && !hasActiveFilters ? (
          <div className="p-8 text-center text-sm text-stone-500">
            No hay colores registrados todavía.
          </div>
        ) : colors.length === 0 && total === 0 ? (
          <div className="flex flex-col items-center gap-2 p-8 text-center text-sm text-stone-500">
            <span>No se encontraron colores con esos filtros.</span>
            <button
              type="button"
              onClick={clearFilters}
          ...
          <table className="min-w-full divide-y divide-stone-200">
            ...
            <tbody className="divide-y divide-stone-100">
              {colors.map((color, index) => (
                <tr key={color.id} className="hover:bg-stone-50/60">
                  ...
                  <td className="px-4 py-3 text-center text-sm text-stone-600">
                    {color.creadoPor
                      ? [color.creadoPor.nombre, color.creadoPor.apellido].filter(Boolean).join(' ') ||
                        color.creadoPor.username
                      : '—'}
                    <span className="block text-xs text-stone-400">
                      {new Date(color.createdAt).toLocaleString('es-AR', {
                        dateStyle: 'short',
                        timeStyle: 'short',
                      })}
                    </span>
                  ...
                  <td className="px-4 py-3 text-center text-sm">
                    <div
                      ...
                      ref={openMenuId === color.id ? triggerRef : null}
                    >
                      <button
                        type="button"
                        onClick={(event) => openMenuFor(color.id, event)}
                      ...
                      {openMenuId === color.id &&
                        ...
                        createPortal(
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleToggleActivo(color)}
        ...
        {!loading && !listError && colors.length > 0 && (
          <div className="flex flex-col items-center justify-between gap-3 border-t border-stone-200 px-4 py-3 text-sm text-stone-500 sm:flex-row">
            <span>
              ...
              {Math.min(page * pageSize, total)} de {total} colores
            </span>
