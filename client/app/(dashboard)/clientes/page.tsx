...
// the inactive one dimmed. Mirrors presupuestos/page.tsx's SortIcon.
...
export default function ClientesPage() {
  ...
  const openMenuFor = (customerId: number, event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const handleToggleActivo = async (customer: CustomerListItem) => {
    closeMenu();
    const activating = !customer.activo;

    if (!activating) {
      const confirmed = await showConfirm({
        title: 'Desactivar cliente',
        text: `¿Seguro que querés desactivar a ${customer.razonSocial}?`,
    ...
    try {
      ...
      showSuccess(
        activating ? 'Cliente activado' : 'Cliente desactivado',
        `${customer.razonSocial} fue ${activating ? 'activado' : 'desactivado'} correctamente.`,
      );
      await loadCustomers();
    } catch (err) {
      showError(
        activating ? 'No se pudo activar el cliente' : 'No se pudo desactivar el cliente',
        err instanceof Error ? err.message : 'No se pudo conectar con el servidor.',
      );
    } finally {
      setTogglingId(null);
                ...
                <tr key={customer.id} className="hover:bg-stone-50/60">
                  ...
                  <td className="px-4 py-3 text-center text-sm">
                    ...
                    >
                      <button
                        type="button"
                        onClick={(event) => openMenuFor(customer.id, event)}
                      ...
                      {openMenuId === customer.id &&
                        ...
                        createPortal(
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleToggleActivo(customer)}
