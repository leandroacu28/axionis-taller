...
- [x] 7.1 Login sets `token`/`user` cookies, no `access_token` in `localStorage`, lands on `/home`.
- [x] 7.2 Middleware matrix: `/` → `/login`; unauth on protected routes → `/login`; auth on `/login` → `/home`.
- [x] 7.3 Dashboard shell renders Header + Sidebar with exactly 3 nav links; all resolve (no 404s).
- [x] 7.4 Header shows name/initials/`rol` from the login user payload.
- [x] 7.5 Dark mode toggles, persists across reload, and survives logout (theme unaffected by session clear).
- [x] 7.6 `prisma migrate dev` applies cleanly; master user (`lmoreno`) has non-null `nombre`, `apellido`, `rol='admin'` after migration.
