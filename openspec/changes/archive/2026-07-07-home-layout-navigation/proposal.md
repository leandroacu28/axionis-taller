...
## Success Criteria
- [x] Login sets cookies, drops localStorage, lands on `/home`.
- [x] Middleware redirects unauthenticated users to `/login` and authenticated users off `/login`; `/` → `/login`.
- [x] Dashboard shell renders Header + Sidebar with the three sections; links resolve (no 404).
- [x] Header shows name/initials/rol from the login user payload.
- [x] Dark mode toggles and persists.
- [x] `User` migration applies cleanly and master user has non-null profile fields.
