...
- [ ] 4.1 Create `client/app/lib/users.ts`: `UserListItem`/`CreateUserPayload`/`UpdateUserPayload` types
- [ ] 4.2 Add `listUsers()` (GET), `createUser()` (POST), `updateUser(id, data)` (PATCH `/users/:id`) — mirror `login()`'s fetch/`res.ok`/throw shape, using `getAuthHeader()`

## Phase 5: Frontend Page

- [ ] 5.1 Rewrite `client/app/(dashboard)/usuarios/page.tsx` (`'use client'`): state for `users`/`loading`/`error`/`editingUser`/`formOpen`/form fields; `useEffect` calls `listUsers()` on mount
- [ ] 5.2 Render table (`username`, `nombre`, `apellido`, `rol`, Editar action) with loading/error/empty states
- [ ] 5.3 Add inline form: "Nuevo usuario" (create, `editingUser=null`) and "Editar" (pre-filled, `username` disabled); `rol` `<select>` with exactly `admin`/`empleado`
- [ ] 5.4 Wire submit: `editingUser ? updateUser(...) : createUser(...)`, refresh list, close form, error banner on failure
- [ ] 5.5 Apply established styling: white table card, slate/stone borders, rose gradient primary buttons, red error banner (per `login/page.tsx`)

## Phase 6: Frontend Manual Verification

- [ ] 6.1 Verify `/usuarios` lists users on load, create adds a row, edit updates a row without full reload, `username` uneditable, loading/error/empty states render without crashing, page reachable by any authenticated `rol`
...
- [ ] 7.2 Walk proposal.md's full Success Criteria checklist end-to-end and confirm each item (pending PR2 — frontend)
