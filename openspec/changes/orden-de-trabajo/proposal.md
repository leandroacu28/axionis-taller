...
### In Scope
- **Backend — new module**: `server/src/ordenes-trabajo/` (controller, service, module, `create-orden-trabajo.dto.ts`, `update-orden-trabajo.dto.ts`, `list-ordenes-trabajo-query.dto.ts`, optionally `export-ordenes-trabajo-query.dto.ts`), registered in `app.module.ts` as `OrdenesTrabajoModule`. Controller route base `ordenes-trabajo`. Structure follows the cleanest recent CRUD example — `server/src/etiquetas/` — thin controller, service owns all Prisma calls, `class-validator` whitelist-safe DTOs, Nest built-in exceptions, Spanish user-facing messages, and a `SELECT` whitelist for the response shape (including nested `cliente`, `vehiculo`, `mecanico`, `tiposServicio`, `creadoPor`, `actualizadoPor` as `{ id, ... }`).
- **Data model — new `OrdenTrabajo` Prisma model** + one additive migration. Fields:
  ...
  - `createdAt`, `updatedAt`.
  - Corresponding back-relations added on `User` (`ordenesTrabajoCreadas`, `ordenesTrabajoActualizadas`, plus the assigned-mechanic back-relation e.g. `ordenesTrabajoAsignadas`), on `Cliente` (`ordenesTrabajo`), and on `Vehiculo` (`ordenesTrabajo`).
- **Data model — extend `TipoServicio`**: add back-relation `ordenesTrabajo OrdenTrabajo[]` (the other side of the many-to-many). Additive, no data change to existing rows.
...
- **Endpoints**: `GET /ordenes-trabajo` (paginated + filtered list), `GET /ordenes-trabajo/:id` (single, for edit), `POST /ordenes-trabajo` (create), `PATCH /ordenes-trabajo/:id` (update). No export, no `DELETE`. All require a valid Bearer token (`JwtAuthGuard`) — no role/permission check.
...
- **Frontend — new route group**: `client/app/(dashboard)/ordenes-trabajo/` with a `page.tsx` (list + table showing `numero`, cliente, vehículo, estado, prioridad, mecánico, fecha) and create/edit UI. Because this is a multi-field transactional form (cascading select, multi-select, enums), the UI follows the **dedicated create/edit pages** pattern the productos section recently moved to (`productos/nuevo`, `productos/editar/[id]`) rather than a single modal. New `client/app/lib/ordenes-trabajo.ts` typed API client (typed interfaces, shared response handler, `getAuthHeader()` on every call, `list`/`get`/`create`/`update` + `searchTiposServicio`-style helpers as needed).
...
- **Frontend — navigation**: a new **top-level** "Orden de Trabajo" entry in `client/app/lib/navigation.tsx` (sibling of Clientes/Vehículos/Productos), **not** nested under "Configuraciones" — it is transactional, not a catalog. Icon: a dedicated `/icons/` asset if added, otherwise a placeholder per existing convention.
...
- `app-navigation`: adds a flat top-level "Orden de Trabajo" entry. No role filtering — consistent with the existing "No Role Filtering in V1" posture.
...
- **D1 — Naming**: English scaffolding (`server/src/ordenes-trabajo/`, `/ordenes-trabajo` route, `client/app/lib/ordenes-trabajo.ts`, `client/app/(dashboard)/ordenes-trabajo/`) with the Prisma model as `OrdenTrabajo`, Spanish DTO field names/messages/UI copy and Spanish enum values (`normal`/`alta`/`urgente`, `pendiente`/`en_proceso`/`terminado`). *Rationale*: mirrors the existing English-scaffolding + Spanish-domain convention.
...
- **D12 — Dedicated create/edit pages (not modal)**: frontend uses `ordenes-trabajo/nuevo` + `ordenes-trabajo/editar/[id]` like productos. *Rationale*: too many fields/controls for a modal.
...
1. Revert the timestamped migration (drop the `OrdenTrabajo` table, its `Estado`/`Prioridad` enums, and its FKs / the `_OrdenTrabajoToTipoServicio` join table) — safe because nothing else references it. **Note**: the odometer-sync side effect has already **overwritten** `Vehiculo.kilometraje` values for affected vehicles; those values are **not** restored by dropping the table (no historical snapshot is kept elsewhere). This is the one non-additive data effect and must be called out at apply time.
2. Remove `server/src/ordenes-trabajo/` and its `app.module.ts` import; remove the `OrdenTrabajo` back-relations from `User`, `Cliente`, `Vehiculo`, and the `ordenesTrabajo` back-relation from `TipoServicio`.
...
4. Remove `client/app/(dashboard)/ordenes-trabajo/`, `client/app/lib/ordenes-trabajo.ts`, the multi-select widget, and the "Orden de Trabajo" nav entry.
...
| Many-to-many relation / back-relations mis-wired (novel on `OrdenTrabajo`) | Med | Mirror Producto↔Etiqueta exactly; `sdd-design` specs the join-table and back-relation names |
...
## Success Criteria
- [ ] `GET /ordenes-trabajo`, `GET /ordenes-trabajo/:id`, `POST /ordenes-trabajo`, `PATCH /ordenes-trabajo/:id` all require a valid Bearer token (401 otherwise) — no role check.
...
- [ ] Frontend `/ordenes-trabajo` lists orders; `/ordenes-trabajo/nuevo` and `/ordenes-trabajo/editar/[id]` provide create/edit with a cascading cliente→vehículo picker (vehículo resets on cliente change), a tipos-de-servicio multi-select, and a searchable mecánico picker.
- [ ] A flat top-level "Orden de Trabajo" nav entry (not under Configuraciones) is visible to any authenticated user.
