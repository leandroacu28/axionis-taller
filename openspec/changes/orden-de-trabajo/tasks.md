# Tasks: Orden de Trabajo (Work Order intake)
...
- `schema.prisma` (2 enums + `OrdenTrabajo` model + 6 back-relations across 4 models): ~50-60 lines
- Migration SQL: ~60-90 lines
- `ordenes-trabajo.service.ts` (`findAll`/`findOne`/`create`/`update`, 3 guards, `$transaction`, `SELECT`/`formatNumero` constants): ~230-260 lines
- `ordenes-trabajo.controller.ts` (4 routes, `@Request()` on POST/PATCH): ~55-65 lines
- 3 DTOs: ~90-100 lines combined
- `ordenes-trabajo.module.ts` + `app.module.ts` edit: ~15 lines
...
- `client/app/lib/ordenes-trabajo.ts`: ~140-160 lines
- `client/app/(dashboard)/ordenes-trabajo/page.tsx`: ~260-300 lines
- `OrdenTrabajoForm.tsx` (cascading select + multi-select + two enums): ~220-260 lines
...
|------|------|-----------|-------|
| 1 | Prisma schema + migration + full `ordenes-trabajo` backend module (DTOs, service, controller, module, `app.module.ts` registration) | PR 1 | ~500-570 lines; foundational, independently curl/Postman-verifiable once migrated; already over the 400-line budget alone — flag for chaining/`size:exception` decision |
...
| 3 | Frontend: `lib/ordenes-trabajo.ts`, `SearchableSelect`/`referenceSelectConfigs` extension, `TipoServicioMultiSelect`, `OrdenTrabajoForm`, list/nuevo/editar pages, nav entry | PR 3 | ~820-940 lines; depends on PR 1 + PR 2 endpoints being live; itself worth splitting further (form+pickers vs. pages) if the reviewer wants smaller diffs |
...
- [x] 1.2 Modify `server/prisma/schema.prisma`: add the `OrdenTrabajo` model exactly as specified in design.md § `OrdenTrabajo` model (id, `numero String? @unique`, `fechaIngreso`, `kilometros`, `prioridad`, `motivoIngreso @db.Text`, `estado`, required `clienteId`/`vehiculoId`/`mecanicoId` FKs, `tiposServicio TipoServicio[]`, nullable `creadoPorId`/`actualizadoPorId` `SetNull` audit FKs, `createdAt`/`updatedAt`)
- [x] 1.3 Modify `server/prisma/schema.prisma`: add the three `User` back-relations (`ordenesTrabajoAsignadas`, `ordenesTrabajoCreadas`, `ordenesTrabajoActualizadas`) and the single back-relations on `Cliente`, `Vehiculo`, `TipoServicio` per design.md § Data Model / Back-relations
...
- [x] 1.5 Run `npx prisma migrate dev --name add_orden_trabajo` in `server/` to generate the additive migration (new table, both enums, `_OrdenTrabajoToTipoServicio` join table) and regenerate the Prisma Client

## Phase 2: Backend — `ordenes-trabajo` Module

- [x] 2.1 Create `server/src/ordenes-trabajo/dto/create-orden-trabajo.dto.ts`: `CreateOrdenTrabajoDto` with the exact field/validator set in design.md § DTO fields (`fechaIngreso?`, `kilometros`, `prioridad?`, `motivoIngreso`, `estado?`, `clienteId`, `vehiculoId`, `mecanicoId`, `tipoServicioIds: number[]` with `@ArrayMinSize(1)`)
- [x] 2.2 Create `server/src/ordenes-trabajo/dto/update-orden-trabajo.dto.ts`: `UpdateOrdenTrabajoDto` — identical field set to create (house pattern, no `PartialType`); neither DTO carries `numero`/`creadoPorId`/`actualizadoPorId`
- [x] 2.3 Create `server/src/ordenes-trabajo/dto/list-ordenes-trabajo-query.dto.ts`: `EstadoFilter` type + `ListOrdenesTrabajoQueryDto` (`page`/`pageSize`, `search?` matching `numero`/`cliente.razonSocial`/`marca.marca`/`marca.modelo`, `estado?: 'all' | 'pendiente' | 'en_proceso' | 'terminado'`) per design.md § `list-ordenes-trabajo-query.dto.ts`
- [x] 2.4 Create `server/src/ordenes-trabajo/ordenes-trabajo.service.ts`: add `ORDEN_TRABAJO_SELECT` and `formatNumero(id)` module-level constants verbatim from design.md § Service
...
- [x] 2.6 Add `findAll(query)` — paginated/filtered list using `ORDEN_TRABAJO_SELECT`, per-`estado` counts (mirrors `etiquetas.findAll`'s `$transaction([findMany, count, ...])` shape, reframed per D2/design.md § `list-ordenes-trabajo-query.dto.ts`)
- [x] 2.7 Add `findOne(id)` — `findUnique` with `ORDEN_TRABAJO_SELECT`; `NotFoundException('Orden de trabajo no encontrada.')` if missing
...
- [x] 2.10 Create `server/src/ordenes-trabajo/ordenes-trabajo.controller.ts`: `@Controller('ordenes-trabajo')`, class-level `@UseGuards(JwtAuthGuard)`, routes `GET /`, `GET /:id`, `POST /`, `PATCH /:id` (no `export`, no `DELETE`); `POST`/`PATCH` inject `@Request()` and pass `req.user.userId` per design.md § Backend Module Structure
- [x] 2.11 Create `server/src/ordenes-trabajo/ordenes-trabajo.module.ts`: `controllers: [OrdenesTrabajoController]`, `providers: [OrdenesTrabajoService]`
- [x] 2.12 Modify `server/src/app.module.ts`: import and register `OrdenesTrabajoModule`
...
- [x] 4.1 Create `client/app/lib/ordenes-trabajo.ts`: typed interfaces, `list`/`get`/`create`/`update` + `searchTiposServicio` helper, `getAuthHeader()` on every call, per design.md § Frontend Component Plan row for this file
...
- [x] 4.4 Create `client/app/(dashboard)/ordenes-trabajo/TipoServicioMultiSelect.tsx`: copy of `EtiquetasMultiSelect.tsx` with `searchEtiquetas` → `searchTiposServicio`, per design.md § Frontend Component Plan
- [x] 4.5 Create `client/app/(dashboard)/ordenes-trabajo/OrdenTrabajoForm.tsx`: shared create/edit form — cliente picker (`clienteSelectConfig`), dependent vehículo picker (`disabled={clienteId === ''}`, `search` closure `(term) => listVehicles({ clienteId, search: term })`, resets `vehiculoId=''` on cliente change), mecánico picker (`mecanicoSelectConfig`), `TipoServicioMultiSelect`, `prioridad`/`estado` enum controls, `fechaIngreso`/`kilometros`/`motivoIngreso` fields, per design.md § DD3 and § Data Flow

## Phase 5: Frontend — Pages & Navigation

- [x] 5.1 Create `client/app/(dashboard)/ordenes-trabajo/page.tsx`: list + table (`numero`, cliente, vehículo, estado, prioridad, mecánico, fecha) with per-`estado` count pills, adapting `productos/page.tsx`'s structure per design.md § Frontend Component Plan
- [x] 5.2 Create `client/app/(dashboard)/ordenes-trabajo/nuevo/page.tsx`: wraps `OrdenTrabajoForm` in create mode
- [x] 5.3 Create `client/app/(dashboard)/ordenes-trabajo/editar/[id]/page.tsx`: loads via `getOrdenTrabajo`, wraps `OrdenTrabajoForm` in edit mode
- [x] 5.4 Modify `client/app/lib/navigation.tsx`: add flat top-level "Orden de Trabajo" entry (`href: '/ordenes-trabajo'`), sibling of Clientes/Vehículos/Productos, **not** nested under Configuraciones's `children`, per D13/design.md § Frontend Component Plan; use `/icons/ordenes-trabajo.svg` if added, else the existing placeholder convention
...
- [ ] 6.2 Verify `GET /ordenes-trabajo`, `GET /ordenes-trabajo/:id`, `POST /ordenes-trabajo`, `PATCH /ordenes-trabajo/:id` all return 401 without a Bearer token
...
- [ ] 6.4 Fire two rapid/concurrent `POST /ordenes-trabajo` requests (e.g. via curl in parallel) and confirm both succeed with **distinct** `numero` values — no collision, no skip
...
- [ ] 6.14 Verify no `DELETE /ordenes-trabajo` route exists and any authenticated `rol` (e.g. `'empleado'`) succeeds identically to `'admin'` on all 4 routes
- [ ] 6.15 Verify the "Orden de Trabajo" nav entry renders as a top-level item (not nested under Configuraciones) and links to `/ordenes-trabajo`
