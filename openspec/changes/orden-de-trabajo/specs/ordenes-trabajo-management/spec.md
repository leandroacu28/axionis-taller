...
- GIVEN an `OrdenTrabajo` row whose `creadoPorId` and/or `actualizadoPorId` point to an existing `User`
- WHEN that `User` row is deleted
- THEN the delete succeeds and the `OrdenTrabajo` row still exists
...
- GIVEN a request to `GET /ordenes-trabajo` has no `Authorization` header or an invalid/expired token
- WHEN the backend handles the request
...
- WHEN `GET /ordenes-trabajo` is called with `estado=pendiente`
- THEN only orders with `estado = 'pendiente'` are returned

### Requirement: Get Single Order

`GET /ordenes-trabajo/:id` MUST require a valid Bearer token and MUST return the whitelisted shape (including nested `cliente`, `vehiculo`, `mecanico`, `tiposServicio`, `creadoPor`, `actualizadoPor`) for the matching `id`. An unmatched `id` MUST return 404.

#### Scenario: Fetch existing order by id

- GIVEN an existing order id and a valid Bearer token
- WHEN `GET /ordenes-trabajo/:id` is called
...
`POST /ordenes-trabajo` MUST require `clienteId`, `vehiculoId`, `mecanicoId`, `kilometros`, `prioridad`, `motivoIngreso`, `estado`, and at least one `tipoServicioId`. `fechaIngreso` MUST default to the current server time when omitted and MUST be accepted and honored when supplied by the client.
...
- WHEN `POST /ordenes-trabajo` is called
...
- WHEN `POST /ordenes-trabajo` is called with that combination
- THEN it returns 400 and no order is created
...
- WHEN `PATCH /ordenes-trabajo/:id` is called with that combination
...
- WHEN `POST /ordenes-trabajo` or `PATCH /ordenes-trabajo/:id` is called
- THEN it returns 400 and no order is created or modified
...
- WHEN `PATCH /ordenes-trabajo/:id` is called with a new `kilometros` value
...
`PATCH /ordenes-trabajo/:id` MUST allow updating any editable field, including transitioning `estado` to any of `pendiente`/`en_proceso`/`terminado` from any current value, in any direction. No linear workflow MUST be enforced and no `cancelado` state exists. An unmatched `id` MUST return 404.
...
`POST /ordenes-trabajo` MUST set `creadoPorId` and `actualizadoPorId` to the authenticated caller's id, resolved server-side from `req.user.userId`. `PATCH /ordenes-trabajo/:id` MUST set `actualizadoPorId` from the caller and MUST NOT modify `creadoPorId`. Neither field is client-settable; any client-supplied value MUST be ignored. `mecanicoId` is a distinct, client-supplied field and MUST NOT be derived from the JWT.
...
- WHEN user B calls `PATCH /ordenes-trabajo/:id`
...
`GET /ordenes-trabajo`, `GET /ordenes-trabajo/:id`, `POST /ordenes-trabajo`, and `PATCH /ordenes-trabajo/:id` MUST authorize solely via `JwtAuthGuard`. This is a deliberate deferral to a future "Permisos" feature.
...
This capability MUST NOT expose a `DELETE /ordenes-trabajo` or `DELETE /ordenes-trabajo/:id` route. Lifecycle is expressed only through `estado`.

#### Scenario: No delete route exists

- GIVEN the `ordenes-trabajo` controller's registered routes
- WHEN inspecting the controller
- THEN no route handles `DELETE /ordenes-trabajo` or `DELETE /ordenes-trabajo/:id`
